...
# Esto evita que la app se cierre si falla una librería
try:
    import gspread
...
# --- VARIABLES GLOBALES ---
# Las definimos afuera para que no se pierdan al cambiar de pantalla
sh = None
ws_jugadoras = None
ws_habilidades = None
ws_asistencia = None
...
def main(page: ft.Page):
    ...
    # Intentamos configurar assets para los PDFs
    ...
    # =========================================================
    #  PANTALLA 1: CONEXIÓN (El "Botón Azul")
    # =========================================================
    ...
    def conectar_google(e):
        # 1. Bloqueamos botón y mostramos carga
        ...
        try:
            # 2. Buscamos credenciales (Raíz o Assets)
            ...
            txt_resultado.value = f"Datos listos: {len(lista_jugadoras_raw)} jugadoras cargadas."
    ...
    # --- ACÁ OCURRE LA MAGIA: Al tocar entrar, cargamos TU CÓDIGO ---
    def ir_al_menu(e):
        page.clean() # Limpiamos la pantalla de conexión
        iniciar_app_completa(page) # Cargamos toda la lógica

    btn_entrar = ft.ElevatedButton("ENTRAR AL SISTEMA", on_click=ir_al_menu, bgcolor="green", color="white", height=50, visible=False)

    # Layout Pantalla Conexión
    pantalla_conexion = ft.Column([
            ft.Icon(ft.Icons.SPORTS_HOCKEY, size=60, color="blue"),
            ft.Text("Hockey App", size=30, weight="bold", color="blue"),
            ft.Divider(),
            lbl_estado, prg_loading, ft.Container(height=20),
            btn_conectar, ft.Container(height=10), txt_resultado,
            ft.Container(height=20), btn_entrar
        ], alignment="center", horizontal_alignment="center")

    page.add(ft.Container(content=pantalla_conexion, alignment=ft.alignment.center, expand=True))


# =========================================================
#  PANTALLA 2: TU APP COMPLETA (FUNCIONES Y VISTAS)
# =========================================================
...
def iniciar_app_completa(page):
    ...
    def vista_formacion():
        ...
        def btn_pdf_click(e):
            if not dd_partido.value: txt_estado_app.value = "⚠️ Falta partido"; page.update(); return
            txt_estado_app.value = "Generando PDF..."
            page.update()
    ...
    def vista_asistencia():
        ...
        def eliminar_datos_dia(e):
            ...
            try:
                ...
                ws_asistencia.clear(); ws_asistencia.append_rows(filas_ok); txt_estado_app.value = "🗑️ Eliminado"; cargar_datos_fecha()
        ...
        def cargar_datos_fecha(e=None):
            f_str = txt_fecha_display.value.replace("📅 ", ""); txt_estado_app.value = f"⏳ Verificando {f_str}..."
            ...
            try:
                ...
                if encontrados > 0: col_lista.visible = False; btn_guardar.visible = False; info_completado.visible = True; txt_estado_app.value = "✅ Registrado"
                else: txt_estado_app.value = "🆕 Nuevo"
        ...
        def guardar(e):
            f_str = txt_fecha_display.value.replace("📅 ", ""); susp = "Suspendido" in dd_tipo.value; txt_estado_app.value = "⏳ Guardando..."; page.update()
            try:
                ...
                txt_estado_app.value = "✅ Guardado"; col_lista.visible = False; btn_guardar.visible = False; info_completado.visible = True; page.update()
            except Exception as ex: txt_estado_app.value = f"Error: {ex}"; page.update()
        ...
        def pdf_click(e):
            try:
                txt_estado_app.value = "Creando Archivo PDF..."
                page.update()
    ...
    def vista_estadisticas_asistencia():
        txt_estado_app.value = "⏳ Calculando..."; page.update()
    ...
    def vista_evaluacion():
        ...
        def mostrar_formulario_evaluacion(dni_jugadora, nombre_jugadora, mes_num):
            ...
            def guardar_y_volver(e):
                ...
                try:
                    ...
                    txt_estado_app.value = "✅ Guardado"; mostrar_lista_jugadoras(mes_num)
                except Exception as ex: txt_estado_app.value = f"Error: {ex}"; page.update()
        ...
        def mostrar_lista_jugadoras(mes_num):
            area_contenido.controls.clear(); txt_estado_app.value = "⏳ Calculando..."; page.update()
            ...
            txt_estado_app.value = "✅ Lista actualizada"; page.update()
    ...
    def vista_plantel():
        def form(jug=None):
            ...
            def save(e):
                if not t_dni.value: txt_estado_app.value = "⚠️ Falta DNI"; page.update(); return
                ...
                try:
                    ...
                    txt_estado_app.value="✅ Guardado"; navegar("plantel")
                except Exception as ex: txt_estado_app.value=str(ex); page.update()
    ...
    def vista_reporte_completo():
        txt_estado_app.value = "📊 Generando reporte general..."; page.update()
        ...
        try:
            ...
            txt_estado_app.value = "✅ Reporte Generado"
    ...
    def vista_resumen_partidos():
        stats_col = ft.Column(scroll="auto", expand=True); txt_estado_app.value="Calculando..."; page.update()
        try:
